    ...
    if rss_channel is not None:
        ...
        if programs is not None:
            for i, program in enumerate(programs):
                ...
                # starttime/stoptime are format "YYYYMMDDHHMMSS ±HHMM" OR just "YYYYMMDDHHMMSS" (assuming UTC)
                # so we retry if parsing with timezone fails
                try:
                    starttime_dt = datetime.strptime(starttime, args.xmltv_datetime_format)
                except ValueError:
                    starttime_dt = datetime.strptime(starttime, DEFAULT_XMLTV_DATETIME_FORMAT_UTC).replace(tzinfo=timezone.utc)
